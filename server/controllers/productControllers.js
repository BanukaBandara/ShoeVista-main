...
export const getProductsByFilter = async (req, res) => {
    try {
        ...
        if (filter === "topBrands") {
            ...
            const brandsToDisplay = ["Adidas", "Nike", "Skechers", "Puma"];
            query = { 
                brand: { $in: brandsToDisplay.map(b => new RegExp(b, 'i')) } 
            };
        } else if (filter === "topRated") {
            
            const products = await Products.find().sort({ rating: -1 }).limit(12);
            return res.status(200).json(products);
        }

        const products = await Products.find(query).limit(12);
        
        if (!products || products.length === 0) {
            return res.status(200).json([]); // Empty array ekak yawanna error ekak wenuwata
        }
        
        res.status(200).json(products);
    } catch (error) {
        console.error('Error in getProductsByFilter:', error.message);
